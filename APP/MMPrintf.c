...
static char* number(char* str, uint32_t* num, int base, int size, int precision, int flags)
{
    ...
    if (flags & FLOAT && (*(float*)(num)) < 0)
    {
        sign = '-';
        *(float*)(num) = -(*(float*)(num));
    }
    else if (flags & SIGN && !(flags & FLOAT) && ((int)(*num)) < 0)
    {
        sign = '-';
        (*num) = -(int)(*num);
    }
    ...
    if (flags & FLOAT)
    {
        uint32_t ival = (uint32_t)(*(float*)num);  // 整数部分
        uint32_t fval = (uint32_t)(((*(float*)num) - ival) * 1000000); // 小数部分

        t = fval % 10;
        if (t != 0) tmp[i++] = digits[t];
        fval /= base;
        t = fval % 10;
        if (t != 0 || i > 0) tmp[i++] = digits[t];
        fval /= base;
        t = fval % 10;
        if (t != 0 || i > 0) tmp[i++] = digits[t];
        fval /= base;
...
int vsprintf(char* buf, const char* fmt, va_list args)
{
    ...
    repeat:
        ...
        case 'm': // mac address
            flags |= SMALL | ZEROPAD;
            ptr = (uint8_t*)va_arg(args, char*);
            for (size_t t = 0; t < 6; t++, ptr++)
            {
                int num = *ptr;
                str = number(str, (uint32_t*)&num, 16, 2, precision, flags);
                *str = ':';
                str++;
            }
            str--;
        ...
        case 'r': // ip address
            ...
            {
                int num = *ptr;
                str = number(str, (uint32_t*)&num, 10, field_width, precision, flags);
                *str = '.';
                str++;
            }
            str--;
...
void MM_printf(const char* fmt, ...)
{
    ...
    if (len > 0) usart1_dma_send((uint8_t*)data, len);
}
